...
type OldCurrentNewLocationBlobMap struct {
	blockList              BlockList
	blockListGrowingPolicy BlockListGrowingPolicy
...
func NewOldCurrentNewLocationBlobMap(blockList BlockList, blockListGrowingPolicy BlockListGrowingPolicy, errorLogger util.ErrorLogger, name string, blockSizeBytes int64, oldBlocksCount, newBlocksCount, initialBlocksCount int) *OldCurrentNewLocationBlobMap {
	...
	for initialOldBlocksCount > 0 && blockListGrowingPolicy.ShouldGrowNewBlocks(0, lbm.newBlocks) {
		initialOldBlocksCount--
		lbm.newBlocks++
	}
	for initialOldBlocksCount > 0 && blockListGrowingPolicy.ShouldGrowCurrentBlocks(lbm.currentBlocks) {
		initialOldBlocksCount--
		lbm.currentBlocks++
	}
...
func (lbm *OldCurrentNewLocationBlobMap) findBlockWithSpace(sizeBytes int64) (int, error) {
	...
	// data.
	for lbm.blockListGrowingPolicy.ShouldGrowNewBlocks(lbm.currentBlocks, lbm.newBlocks) {
