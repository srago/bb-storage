...
func (ba *spannerBlobAccess) keyToDigestWithoutInstanceName(key string) (digest.Digest, error) {
	s := strings.Split(key, "-")
	if len(s) < 2 {
		return digest.BadDigest, fmt.Errorf("Invalid key format (%s)", key)
	}
	in, err := digest.NewInstanceName("")
	if err != nil {
