...
func NewServersFromConfigurationAndServe(configurations []*configuration.ServerConfiguration, registrationFunc func(grpc.ServiceRegistrar), group program.Group) error {
	for _, configuration := range configurations {
		...
		authenticator, needsPeerTransportCredentials, requestTLSClientCertificate, err := grpcauth.NewAuthenticatorFromConfiguration(configuration.AuthenticationPolicy, group)
		if err != nil {
			return err
		}
