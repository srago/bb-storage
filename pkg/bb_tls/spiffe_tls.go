...
func NewMTLSConfigFromClientConfiguration(configuration *configuration.ClientConfiguration) (*tls.Config, error) {
	...
	if configuration.ClientCertificate == "" {
		return nil, fmt.Errorf("MTLS configuration requires a client certificate")
	}
	if configuration.ClientPrivateKey == "" {
		return nil, fmt.Errorf("MTLS configuration requires a client private key")
	}
	if configuration.ServerCertificateAuthorities == "" {
		return nil, fmt.Errorf("MTLS configuration requires certificate authority certs")
	}
	if !util.IsPEMFile(configuration.ClientCertificate) {
		return nil, fmt.Errorf("MTLS client certificate must be stored in a PEM file")
	}
	if !util.IsPEMFile(configuration.ClientPrivateKey) {
	...
	ci, err := newSPIFFECertInfo(configuration.ClientCertificate, configuration.ClientPrivateKey, configuration.ServerCertificateAuthorities)
	if err != nil {
...
func NewMTLSConfigFromServerConfiguration(configuration *configuration.ServerConfiguration, authConfig *grpc_cfg.AuthenticationPolicy) (*tls.Config, error) {
	...
	if configuration.ServerCertificate == "" {
		return nil, fmt.Errorf("MTLS configuration requires a server certificate")
	}
	if configuration.ServerPrivateKey == "" {
		return nil, fmt.Errorf("MTLS configuration requires a server private key")
	}
	if !util.IsPEMFile(configuration.ServerCertificate) {
		return nil, fmt.Errorf("MTLS server certificate must be stored in a PEM file")
	}
	if !util.IsPEMFile(configuration.ServerPrivateKey) {
	...
	ci, err := newSPIFFECertInfo(configuration.ServerCertificate, configuration.ServerPrivateKey, "")
	if err != nil {
